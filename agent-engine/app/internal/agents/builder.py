...
class ToolCallingRunnable:
    def __init__(self, llm: Any, mcp_client: MultiServerMCPClient, server_name: str, allowed_names: Set[str], system_prompt: str, middlewares: list[AgentMiddleware]):
        self.llm = llm
        self.mcp_client = mcp_client
        self.server_name = server_name
        self.allowed_names = allowed_names
    ...
    async def ainvoke(self, message: str) -> str:
        async with self.mcp_client.session(self.server_name) as session:
            ...
            agent = create_agent(self.llm, tools, middleware=[self._middlewares[0]], system_prompt=self.system_prompt)
